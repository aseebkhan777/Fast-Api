...
async def create_lead(user: _schemas.User, db: _orm.Session, lead: _schemas.LeadCreate):
    lead = _models.Lead(**lead.model_dump(), owner_id = user.id ) #it upacks the leads e.g firstname= lead.firstname and lastname = lead.lastname etc..
    db.add(lead)
    db.commit()
    db.refresh(lead)
    return _schemas.Lead.from_orm(lead)

#fxn to get all the leads
async def get_leads(user: _schemas.User, db: _orm.Session):
    leads = db.query(_models.Lead).filter_by(owner_id = user.id)
    return list(map(_schemas.Lead.from_orm, leads)) #loop through all the leads , turn them into lead scehma object and add them to the list 

#helper fxn for updating and deleting lead
...
#fxn to update a Lead
async def update_leaad(lead_id: int, lead:_schemas.LeadCreate, user: _schemas.User, db: _orm.Session):
